...
# All video cards (AGP, PCIE, other) - Used, latest listings
url = 'https://www.trademe.co.nz/a/marketplace/computers/components/video-cards/search?condition=used&sort_order=expirydesc'

# New & used
try:
    if sys.argv[1] == 'new': #If the user passed in new as the second argument when launching the script, change the url 
        url = 'https://www.trademe.co.nz/a/marketplace/computers/components/video-cards/search?sort_order=expirydesc'
except IndexError:
    pass
...
file_name = datetime.now().strftime("%Y%m%d_%H%M") +' GPUS.csv'
